...
import { actualDiameter, APP_VERSION, customBoardShape, defaultProject, newHole, parseProject, patternHoles, presets, SOURCE_URL, GENERATOR_SOURCE_URL, threadChamferSize, threadEnvelopeRadius, validate, type Project, type Hole, type Face } from './domain/project';
      ...
      <div className="workspace">
        <aside className="editor">
          ...
          {p.kind === 'board' ? <section className="panel">
            ...
            <div className="segmented modes" role="group" aria-label={t('boardSource')}>{(['template', 'custom'] as const).map(source => <button key={source} aria-pressed={p.boardSource === source} data-testid={`board-source-${source}`} onClick={() => update(d => { d.boardSource = source; if (source === 'custom') d.orientation = { frontBack: false, upDown: false }; })}>{t(source === 'template' ? 'builtInTemplate' : 'customBoard')}</button>)}</div>
            {p.boardSource === 'template' ? <>
            ...
            </> : <div className="custom-board-fields" data-testid="custom-board-fields">
              <div className="field-grid">{field('customBoardWidth', p.customBoard.width, (d, v) => { d.customBoard.width = v; })}{field('customBoardHeight', p.customBoard.height, (d, v) => { d.customBoard.height = v; })}{field('customBoardThickness', p.customBoard.thickness, (d, v) => { d.customBoard.thickness = v; })}{field('customBoardRadius', p.customBoard.radius, (d, v) => { d.customBoard.radius = v; })}</div>
              <p className="thread-spec">{t('customShape')}: {t(customBoardShape(p.customBoard.width, p.customBoard.height, p.customBoard.radius))}</p>
              <p className="help">{t('customBoardHint')}</p>
              <div className="print-option"><Check label={t('outerLightTrap')} checked={p.customBoard.outerLightTrap.enabled} onChange={v => update(d => { d.customBoard.outerLightTrap.enabled = v; })} testId="outer-light-trap" />
                {p.customBoard.outerLightTrap.enabled && <div className="field-grid">{field('outerLightTrapWidth', p.customBoard.outerLightTrap.width, (d, v) => { d.customBoard.outerLightTrap.width = v; })}{field('outerLightTrapHeight', p.customBoard.outerLightTrap.height, (d, v) => { d.customBoard.outerLightTrap.height = v; })}</div>}<p>{t('outerLightTrapHint')}</p></div>
              <div className="print-option"><Check label={t('innerLightTrap')} checked={p.customBoard.innerLightTrap.enabled} onChange={v => update(d => { d.customBoard.innerLightTrap.enabled = v; })} testId="inner-light-trap" />
                {p.customBoard.innerLightTrap.enabled && <div className="field-grid">{field('innerLightTrapWidth', p.customBoard.innerLightTrap.width, (d, v) => { d.customBoard.innerLightTrap.width = v; })}{field('innerLightTrapHeight', p.customBoard.innerLightTrap.height, (d, v) => { d.customBoard.innerLightTrap.height = v; })}{field('innerLightTrapRadius', p.customBoard.innerLightTrap.radius, (d, v) => { d.customBoard.innerLightTrap.radius = v; })}{field('innerLightTrapDepth', p.customBoard.innerLightTrap.heightMm, (d, v) => { d.customBoard.innerLightTrap.heightMm = v; })}</div>}<p>{t('innerLightTrapHint')}</p></div>
            </div>}
        ...
        <section className="preview-area">
          ...
          <div className="preview-bottom">
            <section className="plot-panel"><h2>{t('plot')}<small>XY / mm</small></h2><HolePlot project={p} radius={p.kind === 'board' ? (p.boardSource === 'custom' ? Math.min(p.customBoard.width, p.customBoard.height) / 2 : selected.editableRadius) : p.flange.diameter / 2} bounds={model?.bounds} onPosition={(x, y) => update(d => { d.central.x = x; d.central.y = y; })} label={t('selectPosition')} /><p className="help">{t(p.kind === 'board' && p.boardSource === 'custom' ? 'customProtectedHint' : 'protectedHint')}</p></section>
