...
export interface Project {
  schemaVersion: 4; generatorVersion: string; license: 'GPL-3.0-only'; units: 'mm';
...
export const defaultProject = (): Project => ({
  schemaVersion: 4, generatorVersion: APP_VERSION, license: 'GPL-3.0-only', units: 'mm',
...
export function parseProject(input: unknown, templateIds: string[]): Project {
  ...
  const { chamfer: _chamfer, ...legacyThread } = expected.central.thread;
  const legacyExpected = { ...expected, central: { ...expected.central, thread: legacyThread } };
  ...
  if (!enumOK || p.schemaVersion !== 4 || !['pitch', 'custom'].includes(p.central.thread.chamfer.mode) || !['roundedRectangle', 'circle'].includes(p.relief.shape) || validate(p).length) throw Error('invalidProject');
