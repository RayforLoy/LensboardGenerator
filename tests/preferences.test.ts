...
it('registers every provided STEP file with unchanged byte hashes', () => {
  const files = readdirSync('steps').filter(f => /\.(step|stp)$/i.test(f)).sort();
  expect(templates).toHaveLength(files.length); expect(templates.map(t => t.file).sort()).toEqual(files);
  expect(new Set(templates.map(t => t.id)).size).toBe(files.length);
