...
it('registers exactly the twelve provided STEP files with unchanged byte hashes', () => {
  const files = readdirSync('steps').filter(f => /\.(step|stp)$/i.test(f)).sort();
  expect(templates).toHaveLength(12); expect(templates.map(t => t.file).sort()).toEqual(files);
