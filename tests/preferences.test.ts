...
it('registers exactly the thirteen provided STEP files with unchanged byte hashes', () => {
  const files = readdirSync('steps').filter(f => /\.(step|stp)$/i.test(f)).sort();
  expect(templates).toHaveLength(13); expect(templates.map(t => t.file).sort()).toEqual(files);
  expect(new Set(templates.map(t => t.id)).size).toBe(13);
