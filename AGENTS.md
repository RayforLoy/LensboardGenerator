...
本文件是仓库级代理协作说明（用户请求的 `agents.md`，使用规范名称 `AGENTS.md`，便于在 Linux / GitHub 上识别）。项目已有实验性 v0.1.0 网页；验证与未完成需求见 `docs/IMPLEMENTATION_STATUS.md`。
...
- 用户已确认 6 个初始 STEP 为自绘，并选择 GPL-3.0-only 用于网页原创代码、模板及基于它们的衍生输出。遵循 `LICENSE` 和 `docs/LICENSING.md`，第三方许可不改写。
