...
状态：本地源码 v0.3.2 新增维护者模板管理器；公开网页仍为已验证版本 v0.3.1，schema 5 不变。见 [实施状态](IMPLEMENTATION_STATUS.md) 与部署记录。本文件保留完整目标架构，未完成项及验证见 [实施状态](IMPLEMENTATION_STATUS.md)。需求以 [PRD](../PRD.md) 为准，不隐藏降级。
...
v0.3.2 的 `scripts/template_manager_gui.py` 使用标准库 Tkinter；不参与网页构建或运行。界面选择一个 `.step` / `.stp` 文件并填写注册元数据，再调用单文件 Node/Replicad 检查器；检查器只读传入路径，报告原字节 SHA-256 / STEP header 和标准化后 B-rep 的有效性、主/附属实体、包围盒及中心当地厚度。

登记流程采用预检 → 生成本地变更 → 显示 Git diff → 用户确认提交推送。只允许从干净的 `main` 工作区开始；调用 Git 使用参数数组、不经过 shell，暂存路径逐项列出。更新的每个文件在写前留取字节快照，写入失败即恢复；推送失败保留提交和文件状态并展示错误，不运行清理、重置或覆盖。流程不使用 GitHub API / token，依赖本机 `git` 凭证和 pnpm / Node。维护器不自动宣称模板兼容或改变 schema。
