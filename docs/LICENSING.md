...
- 用户明确授权并登记的 steps 自绘 STEP 采用 GPL-3.0-only；后续模板必须单独确认发布权，并记录来源及许可，不凭工具勾选框推定第三方权利。
...
Original project content, user-authorized and registered STEP templates, and their derivative designs use GPL-3.0-only. Third-party components and future user-uploaded assets retain their actual terms. The generator's license alone does not relicense every output.
