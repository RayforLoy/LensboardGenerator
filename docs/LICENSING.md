...
- 用户明确授权的 steps 自绘 STEP（初始六个及后续新增七个）采用 GPL-3.0-only；后续登记模板须明确记录作者、来源及许可。
...
Original project content, the thirteen user-authored STEP templates (six initial plus seven additions) and their derivative designs use GPL-3.0-only. Third-party components and future user-uploaded assets retain their actual terms. The generator's license alone does not relicense every output.
